...
class AuthController extends Controller
{
    ...
    public function attemptLogin()
    {
        ...
        if (! $this->auth->attempt([$type => $login, 'password' => $password], $remember)) {
            return redirect()->to(route_to('loginpage'))->withInput()->with('error', $this->auth->error() ?? lang('Auth.badAttempt'));
        }
    ...
    public function attemptRegister()
    {
        ...
        // Skip activation-related steps if activation is not required
        if ($this->config->requireActivation === null) {
            $this->config->requireActivation === null ? $user->activate() : $user->generateActivateHash();
            // Do not generate an activation hash
            // Do not activate the user
            // Do not send an activation email
        } else {
            $this->config->requireActivation === null ? $user->activate() : $user->generateActivateHash();
        ...
        // Skip sending activation message to email if activation is not required
        if ($this->config->requireActivation !== null) {
            // Success!
            $user->activate();

       
            return redirect()->route('loginpage')->with('message', lang('Auth.activationSuccess'));
        }
        $user->activate();

       
        // Success!
        return redirect()->route('loginpage')->with('message', lang('Auth.registerSuccess'));
    }
